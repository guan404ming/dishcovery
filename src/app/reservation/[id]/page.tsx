...
export default function Reservation({params: { id }}: ReservationPageProps) {
    ...
    // 找出符合 id 的 post
    const users = mockResrvationUser.filter((user) => user.productId === parseInt(id));
  ...
  return (
    <>  
        <div className="flex items-center gap-x-4 pl-2 lg:p-4 flex-row justify-between">
            <div className="flex flex-row items-center gap-x-4">
                <img src={`/${post?.photo}`} alt={post?.dish} className="w-12 h-12 rounded-full border-2"/>
                <div className="flex flex-col">
                    <p className="text-md lg:text-lg font-bold">{post?.dish}</p>
        ...
        <div className="flex flex-row items-center justify-between">
            <p className="text-lg lg:text-2xl font-bold">Reservation({users.length})</p>
            <Button
            variant="ghost"
            size="icon"
            className="flex flex-col items-center justify-center"
            onClick={handleCancelClick}
                ><p className="text-lg lg:text-2xl"><AiOutlineMinusCircle/></p>
            </Button>
            {cancelDialogOpen && (
            <ResStateDialog
