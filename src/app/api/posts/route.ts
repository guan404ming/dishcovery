...
export async function POST(request: NextRequest) {
  ...
  const { user_id, title, description, location, dishName, quantity, category } = data as z.infer<
    typeof createCommentRequestSchema
  >;

  try {
    console.log("Creating comment", user_id, title, description, location, dishName, quantity, category);
  } catch (error) {
    console.log(error);
