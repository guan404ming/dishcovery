    ...
    <>
      ...
      <div className="flex flex-row items-center">
        ...
        <div className="flex-grow text-center">
          <p className="text-xl font-bold lg:text-4xl">{post?.title}</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-x-4 pl-2 lg:p-4">
        ...
        <div className="flex flex-col justify-start">
          <p className="text-md font-bold lg:text-lg">{post?.author}</p>
      ...
      {/* dish */}
      {/* 大於 lg 尺寸的布局 */}
      <div className="hidden flex-row items-start justify-start gap-x-4 p-4 lg:flex lg:gap-x-8">
        <img
          src={`/${post?.photo}`}
          alt={post?.title}
          className="h-36 w-36 object-cover lg:h-72 lg:w-72"
        />
        <div className="flex flex-col justify-start p-2 lg:p-4">
          <p className="text-lg font-bold lg:text-xl">{post?.dish}</p>
          <p className="text-md pt-2 text-green-500 lg:text-lg">
            剩餘: {post?.remaining}
          </p>
          <div className="h-36"></div>
          <div className="flex flex-row gap-x-8">
            <Button className="lg:text-md mt-2 w-36 text-xs">Edit</Button>
            <Link href={`/reservation/${id}`}>
              <Button className="lg:text-md mt-2 w-36 text-xs">
                Reservation
              </Button>
            </Link>
          </div>
      ...
      {/* 小於 lg 尺寸的布局 */}
      <div className="flex flex-row items-start justify-start gap-x-4 pl-2 lg:hidden">
        <img
          src={`/${post?.photo}`}
          alt={post?.title}
          className="h-36 w-36 object-cover lg:h-72 lg:w-72"
        ...
        <div className="flex flex-col justify-start p-2">
          ...
          <p className="text-md pt-2 text-green-500">剩餘: {post?.remaining}</p>
        </div>
      </div>
      <div className="flex flex-row items-center justify-center gap-x-4 lg:hidden">
        <Button className="mt-2 w-24 text-xs">Edit</Button>
        <Button className="mt-2 w-24 text-xs">Reservation</Button>
      </div>
      {/* post content */}
      <div>
        <p className="p-2 text-lg text-muted-foreground lg:p-4 lg:text-xl">
          {post?.context}
        </p>
      </div>
    </>
