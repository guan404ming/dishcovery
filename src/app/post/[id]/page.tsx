...
export default async function Post({params: { id }}: PostPageProps) {
    const mockPost = [
        {
          id: 123,
          title: "Post A",
  ...
  return (
    <>  
        {/* top photo */}
        <div>
            <img src={`/${post?.photo}`} alt={post?.title} className="w-full h-36 lg:h-48 object-cover"/>
        </div>
        {/* title */}
        <div className="flex flex-row items-center">
            <div>
                <Link href="/info"><p className="text-xl lg:text-2xl"><FaAngleLeft /></p></Link>
            </div>
            <div className="flex-grow text-center">
                <p className="text-xl lg:text-4xl font-bold">{post?.title}</p>
            </div>
        </div>
        {/* author, date, location */}
        <div className="flex items-center gap-x-4 pl-2 lg:p-4">
            <img src={`/${post?.authorphoto}`} alt={post?.author} className="w-12 h-12 rounded-full border-2"/>
            <div className="flex justify-start flex-col">
                <p className="text-md lg:text-lg font-bold">{post?.author}</p>
                <p className="text-md text-muted-foreground">{post?.date} | {post?.place}</p>
            </div>
        </div>
        {/* dish */}
        {/* 大於 lg 尺寸的布局 */}
        <div className="hidden lg:flex flex-row items-start gap-x-4 lg:gap-x-8 justify-start p-4">
            <img src={`/${post?.photo}`} alt={post?.title} className="w-36 h-36 lg:w-72 lg:h-72 object-cover"/>
            <div className="flex flex-col justify-start p-2 lg:p-4">
                <p className="text-lg lg:text-xl font-bold">{post?.dish}</p>
                <p className="text-md lg:text-lg text-green-500 pt-2">剩餘: {post?.remaining}</p>
        ...
        {/* 小於 lg 尺寸的布局 */}
        <div className="flex flex-row items-start gap-x-4 justify-start lg:hidden pl-2">
            <img src={`/${post?.photo}`} alt={post?.title} className="w-36 h-36 lg:w-72 lg:h-72 object-cover"/>
            <div className="flex flex-col justify-start p-2">
                <p className="text-lg font-bold">{post?.dish}</p>
                <p className="text-md text-green-500 pt-2">剩餘: {post?.remaining}</p>
            </div>
        </div>
        <div className="flex flex-row items-center justify-center gap-x-4 lg:hidden">
            <Button className="mt-2 text-xs w-24">Edit</Button>
            <Button className="mt-2 text-xs w-24">Reservation</Button>
        </div>
        {/* post content */}
        <div>
            <p className="text-lg lg:text-xl text-muted-foreground p-2 lg:p-4">{post?.context}</p>
        </div>
    </>
  );
}
